...
if [ "$USEKEYVAULT" = "Yes" ]; then
	...
	az identity federated-credential create --name federatedIdentityName --identity-name $akskvidentityClientId --resource-group $RESOURCEGROUPNAME --issuer $OIDC_ISSUER --subject system:serviceaccount:profisee:profiseeserviceaccount --audience api://AzureADTokenExchange
